// Adapted for direct browser usage without bundler:
// Use React global and UMD for @use-gesture/react
const { useEffect, useMemo, useRef, useCallback } = React;
const { useGesture } = window.ReactUseGesture || {};
// Fallback when gesture library fails to load; dome still renders
const useGestureSafe = useGesture || ((...args) => () => {});
// Styles are linked from collage.html via dome-gallery.css
...
function DomeGallery({
  ...
  // Native pointer-based drag rotation (no external library)
  useEffect(() => {
    ...
    const onPointerUp = e => {
      ...
      const vx = clamp(((e.clientX - lastMove.x) / dt) * 0.6, -1.2, 1.2);
      const vy = clamp(((e.clientY - lastMove.y) / dt) * 0.6, -1.2, 1.2);
...
// Expose globally for Babel inline usage
